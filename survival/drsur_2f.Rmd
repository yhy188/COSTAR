...
library("factorial2x2")
 dfsim<-data.frame(simdata)
 indA <- dfsim[, 'indA']    # treatment A indicator
indB <- dfsim[, 'indB']    # treatment B indicator
 ########
time <- dfsim[, 'time']    # follow-up time
#time<-time*(0.6+1.2*indA)*(0.3+1.3*indB)
  dfsim$time<-time
###########

###############Causal 
event <- dfsim[, 'event']  # event indicator
nAc<-nrow(dfsim[dfsim$indA==1,])
#Aprocess<-rbinom(nAc, 1,.45)
###For significient in ANOVA and solution of the blocks (dividually)
#write.csv(Aprocess,"Aprocess.csv")
Aprocess<-read.csv("Aprocess.csv",header = TRUE)
sum(dfsim[dfsim$indA==1,"event"])
#dfsim[dfsim$indA==1,"event"]<-dfsim[dfsim$indA==1,"event"]*Aprocess
dfsim[dfsim$indA==1,"event"]<-dfsim[dfsim$indA==1,"event"]*Aprocess$x
sum(dfsim[dfsim$indA==1,"event"])


nBc <- nrow(dfsim[dfsim$indB==1,])
#Bprocess<-rbinom(nBc, 1,.65)
#write.csv(Bprocess,"Bprocess.csv")
Bprocess<-read.csv("Bprocess.csv",header = TRUE)

sum(dfsim[dfsim$indB==1,"event"])
#dfsim[dfsim$indB==1,"event"]<-dfsim[dfsim$indB==1,"event"]*Bprocess
dfsim[dfsim$indB==1,"event"]<-dfsim[dfsim$indB==1,"event"]*Bprocess$x
sum(dfsim[dfsim$indB==1,"event"])
###########
covmat <- dfsim[, 6:10]    # baseline covariates
...
ABc<-head(df[df$indA==1 &(df$indB==1)&(df$event==1),],1)
ABc<-head(df[df$indA==1 &(df$indB==1)&(df$event==0),],1)
AcBc<-head(df[df$indA==0&(df$indB==1)&(df$event==1),],1)
AcBc<-head(df[df$indA==0&(df$indB==1)&(df$event==0),],1)
ABce<-head(df[df$indA==1&(df$indB==0)&(df$event==1),],1)
ABcec<-head(df[df$indA==1&(df$indB==0)&(df$event==0),],1)
AcBc<-head(df[df$indA==0&(df$indB==0)&(df$event==1),],1)
AcBc<-head(df[df$indA==0&(df$indB==0)&(df$event==0),],1)
AB.samp<-rbind(ABc,ABc,AcBc,AcBc,ABce,ABcec,AcBc,AcBc)
library("xtable")
xtable(AB.samp)
```
...
##
```{r}
#ps <- glm(T ~ cvd +fac2+ fac3+fac4+fac5,data = sur.df)
#plot(density(ps$fitted.values))
table(sur.df$T)
#sur.df$ps<-round(ps$fitted.values,3)
library("nnet")
ps <- multinom(T ~cvd +fac2+ fac3+fac4+fac5,data = sur.df)
table(ps$fitted.values)
...
{
  plot(density(sur.df[sur.df$T==1,"ps"]),ylim=range(0,78)
...
plot(survfit(Surv(cbd.df$time, cbd.df$event) ~1,#~ T,
             data=cbd.df))
...
#  legend("bottomleft", legend=c("A1c A2c", "A1c A2", "A1 A2c", "A1 A2"),
          legend("bottomleft", 
...
# xlab = expression(eta^s), ylab = expression(eta^s))
```
#####Samples of 16 cases
```{r}
...
#      legend("right", legend=c("resp1.m", "resp2.m", "resp1.se", "resp2.se"),
 # legend(1,0.6, legend=c("median", "dispersion"),
     legend("topleft", legend=c("median", "dispersion"),
    col=c(1:4) , 
    lwd=2)
  }
  grid()
...
for (i in seq(1:3)) {
   print("###############LR with dual responses################",i)
  if(i==1|i==2){
      names(ggplot.df)<-c("1","2","3","4")
    plot.mat<-ggplot.df
     ind<-i+1
  ...
  ##LR
  lr.mat<-lm(value~x1+x2+Var2, data = df.lr)
 lr.mat$coefficients 
 summary(lr.mat)
...
for (bi in seq(1:4)) {
  if(bi==1){
    sur.vdf[sur.vdf$block==bi,"Toptv"]<-0.92
  }else if(bi==2){
    sur.vdf[sur.vdf$block==bi,"Toptv"]<-0.93
  }else if(bi==3){
    sur.vdf[sur.vdf$block==bi,"Toptv"]<-0.95
  }else {
    sur.vdf[sur.vdf$block==bi,"Toptv"]<-0.84
  }
}
...
for (bi in seq(1:4)) {
  if(bi==1){
    sur.vdf$Tdopt<-4
    sur.vdf[sur.vdf$block==bi,"Tdoptv"]<-0.92
  }else if(bi==2){
    sur.vdf$Tdopt<-4
    sur.vdf[sur.vdf$block==bi,"Tdoptv"]<-0.93
  }else if(bi==3){
    sur.vdf$Tdopt<-4
    sur.vdf[sur.vdf$block==bi,"Tdoptv"]<-0.95
  }else {
    sur.vdf$Tdopt<-3
    sur.vdf[sur.vdf$block==bi,"Tdoptv"]<-0.89
  }
}
...
head(sur.vdf)
{
  ...
  abline(h =c(mean.global[4]), col=c(2), lwd=2, lty=2)#
...
#  grid()
  ...
  legend("bottomright", legend=c("T=3", "T=4","CDF","Quantile"),
...
legend("bottomleft",  legend=( expression({c(bar(A1),bar(A2))}, 
                                     {c(bar(A1),A2)},
                                    {c(A1,bar(A2))},
                                    {c(A1,A2)})
                           ),
 ...
 # line.df<-ggplot.df[,1:3]
  ...
  names(line.df)<-c("B1","B2","B3","B4")  
  {
    ...
    maxr<-max(line.df)+0.15
      plot(x=seq(1:4),as.numeric(line.df[,1]),ylim = range(minr,maxr), 
...
#       cex = 2, col = c(bi),      bg = c(bi+4), lwd=2, lty=c(bi),    bty = "l")
  ...
  ####(4,4)-->(3,4)
  for (ti in seq(1:4)) {
        points(x = ti, y = line.dfrepmat[4,ti],  col = "brown",lwd=3, #bg = "blue", 
               pch = 20, cex = 2)
    #COSTAR; VM
 points(x = 4, y = line.dfrepmat[3,4],  col = "brown", lwd=3,bg = "blue", pch = 22, cex = 2)
  points(x = 4, y = line.dfrepmat[4,4],  col = "brown",lwd=3, bg = "white", pch = 21, cex = 2)
  
  
##  meanvm<-mean(line.dfrepmat[4,])
##  abline(h =meanvm, col=c(10), lwd=1, lty=1)
    ####(4,4)-->(3,4)
##    meancvm<-mean(c(line.dfrepmat[4,c(1,2,3)]),line.dfrepmat[4,4])
...
legend("bottomright",  legend=c( "VM", "COSTAR"),
       #legend=c("Block1", "Block2","Block3","Block4"),
       col=c(5,6), lty=c(5,6),  cex=0.8)
}
